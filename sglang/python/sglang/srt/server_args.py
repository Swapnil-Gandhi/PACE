...
class ServerArgs:
    ...
    # PACE (arXiv:2605.06914): per-step branch-admission control.
    pace_enable: bool = False
    pace_mode: str = "pace"
    pace_cap: Optional[int] = None
    pace_rho: float = 0.8
    pace_tpot_slo_ms: float = 50.0
    pace_utility: str = "linear"
    pace_window: int = 200
    pace_refit_interval: float = 600.0
    pace_coeffs_path: Optional[str] = None
    pace_context_mode: str = "raw"
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--pace-enable",
            action="store_true",
            help="Enable PACE per-step branch-admission control.",
        )
        parser.add_argument(
            "--pace-mode",
            type=str,
            default=ServerArgs.pace_mode,
            help="PACE admission policy: off | cap | eager | pace (or a registered custom policy).",
        )
        parser.add_argument(
            "--pace-cap",
            type=int,
            default=ServerArgs.pace_cap,
            help="Per-request branch width cap for --pace-mode cap (e.g. 2, 5).",
        )
        parser.add_argument(
            "--pace-rho",
            type=float,
            default=ServerArgs.pace_rho,
            help="Fraction of slack budget PACE may spend, in (0,1]. Default 0.8.",
        )
        parser.add_argument(
            "--pace-tpot-slo-ms",
            type=float,
            default=ServerArgs.pace_tpot_slo_ms,
            help="TPOT SLO target in ms used for PACE deadlines/slack.",
        )
        parser.add_argument(
            "--pace-utility",
            type=str,
            default=ServerArgs.pace_utility,
            help="PACE branch utility: linear | concave | priority.",
        )
        parser.add_argument(
            "--pace-window",
            type=int,
            default=ServerArgs.pace_window,
            help="Rolling window size for PACE online latency-predictor refit.",
        )
        parser.add_argument(
            "--pace-refit-interval",
            type=float,
            default=ServerArgs.pace_refit_interval,
            help="Seconds between PACE online OLS refits of the latency predictor.",
        )
        parser.add_argument(
            "--pace-coeffs-path",
            type=str,
            default=ServerArgs.pace_coeffs_path,
            help="Path to offline-calibrated PACE latency coefficients json {a,b,c}.",
        )
        parser.add_argument(
            "--pace-context-mode",
            type=str,
            default=ServerArgs.pace_context_mode,
            help="PACE L_context aggregation mode: raw | dedup.",
        )
        parser.add_argument(
            "--cpu-offload-gb",
            type=int,
